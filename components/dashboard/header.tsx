  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-background/95 backdrop-blur-sm border-b border-border px-6 flex items-center justify-between gap-4">
      ...
      <div className={cn(
        ...
        />
        <kbd className="hidden lg:inline-flex px-2 py-0.5 rounded bg-background text-xs text-muted-foreground border border-border">
          Ctrl+K
        </kbd>
      </div>
      ...
      <div className="flex items-center gap-3 ml-auto">
        ...
        <div className={cn(
          ...
          user?.plan === "premium" ? "bg-[#F5C842]/15 text-[#F5C842]" :
          user?.plan === "pro" ? "bg-primary/15 text-primary" :
          "bg-secondary text-muted-foreground"
        )}>
          {user?.plan === "premium" ? <Crown className="w-3.5 h-3.5" /> : <Zap className="w-3.5 h-3.5" />}
